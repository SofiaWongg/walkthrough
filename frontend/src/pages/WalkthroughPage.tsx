    ...
    const createAndStart = () => {
      ...
      recognition.onresult = (event: SpeechRecognitionEvent) => {
        ...
        for (let i = event.resultIndex; i < event.results.length; i++) {
          ...
          if (result.isFinal) {
            // Guard against Android Chrome setting resultIndex=0 on every event,
      ...
      recognition.onend = () => {
        if (isListeningRef.current && recognitionRef.current === recognition) {
          ...
          // Cancel the silence timer and flush any pending text before restarting.
          // Android Chrome re-delivers recent final results on every new session,
          // so we must start each session with a clean pendingTextRef.
          if (silenceTimerRef.current) clearTimeout(silenceTimerRef.current);
